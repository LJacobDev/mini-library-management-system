...
- **Visual accessibility**: Maintain both light and dark themes, validating color contrast during each milestone. Archive the current black+green palette (`docs/dev/style-archive.md`) and plan a library-appropriate palette overhaul soon.
...
7. **Core circulation & admin endpoints** (Phase B)
   ...
   - [ ] Reintroduce dashboard layout for signed-in users with quick links to loans, reservations, admin catalog.
...
8. **Interactive librarian/member surfaces** (Phase B continuation)
   ...
   - [ ] Scaffold `/desk/checkout` stepper and `/admin/media` forms using Nuxt UI tables/forms tied to the new endpoints.

9. **Deferred enhancements** (Phase C)
   - [ ] Polish AI assistant UI, add rate limiting as needed.
   - [ ] Add SQLite fallback demo (optional) and toggle.
   - [ ] Wire CI smoke tests against `/api/check/*`, `/api/catalog`.
   - [ ] Capture screenshots/video walkthrough, accessibility sweep, and documentation sync (`spec-final`, `plans-ci-cd-*`).
