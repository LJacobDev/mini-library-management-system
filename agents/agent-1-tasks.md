...
- [ ] Cross-check `docs/data/schema.sql` against `spec-final.md` §3.3/§9 and note any discrepancies before editing.
- [ ] Run `supabase db diff` to generate migrations from the confirmed schema and save them to `db/migrations/<timestamp>_*.sql`.
- [ ] Add or update an `npm run db:migrate:local` script (wrapping the Supabase CLI) and document the command in `README.md` in coordination with Agent 3.
- [ ] Scaffold `scripts/db/seed.ts` with the schema-driven contract, include TODO markers for real seed data, and drop lightweight JSON fixtures Agent 2 can consume meanwhile.
...
- [ ] Update or create `docs/dev/env.md` describing which variables need real secrets vs. demo placeholders and link to Supabase/Vercel setup steps.
- [ ] Notify Agent 3 (in their context/tasks) when env changes land so CI secrets stay aligned.
...
- [ ] Ship `GET /api/catalog` + `/api/catalog/:id` with pagination, filters, queue summary, cache headers, and integration tests validating schema + conflict codes.
- [ ] Implement admin catalog mutations with optimistic locking (`If-Match`), duplicate handling, and audit log persistence.
- [ ] Deliver `/api/me/loans` + `/api/me/reservations` with SSR-friendly pagination metadata and comprehensive tests.
- [ ] Extend `scripts/db/seed.ts` with catalog/loan/reservation demo entries and record seeded credentials in `docs/dev/backend-notes.md`.
...
- [ ] Ship `PATCH /api/loans/:id/renew` and `POST /api/loans/:id/override`, surfacing typed conflict codes for policy failures.
- [ ] Build `POST /api/loans/checkin` with reservation queue reconciliation and `handoff` metadata in the response.
- [ ] Cover checkout/renew/override/checkin flows with integration tests (replay, locking, queue advancement) and write audit entries.
...
- [ ] Implement reservation endpoints (`POST`, `DELETE`, `claim`, `advance`) with locking + idempotent replay safeguards.
...
- [ ] Re-audit RLS, response envelopes, and edge-case tests; regenerate OpenAPI artifacts if needed.
